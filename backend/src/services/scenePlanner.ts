...
export async function buildScene() {
  ...
  const transcription =
    enrichTranscription(rawTranscription);
  ...
  const semanticPose =
    await semanticExtractionPipeline(
      sentenceSegments
    );
  ...
  fs.writeFileSync(
    path.join(outputDir, "02_transcription.json"),
    JSON.stringify(transcription, null, 2)
  );

  fs.writeFileSync(
    path.join(outputDir, "03_semantic_pose.json"),
    JSON.stringify(semanticPose, null, 2)
  );

  fs.writeFileSync(
    path.join(outputDir, "04_narrative_scenes.json"),
    JSON.stringify(narrativeScenes, null, 2)
  );

  fs.writeFileSync(
    path.join(outputDir, "05_selected_widgets.json"),
    JSON.stringify(selectedWidgets, null, 2)
  );

  fs.writeFileSync(
    path.join(outputDir, "06_scene_config.json"),
