...
// --------------------------------------------------------
//
function determineNarrativeSection(index, total) {
...
function segmentTranscriptionIntoSentences(words, fps = 30) {
    ...
    for (let i = 0; i < words.length; i++) {
        ...
        const shouldCloseSentence = isSentenceBoundary(word.word) || isLastWord;
...
// --------------------------------------------------------
//
function buildPrompt(segment, narrativeSection) {
    return `
...
async function extractSemanticMeaning(segment, index, total) {
    ...
    const response = await openai.chat.completions.create({
        ...
        messages: [
            {
                role: 'system',
                content: 'You are a semantic cinematography extraction engine.',
            },
            {
                role: 'user',
                content: prompt,
    ...
    return {
        ...
        narrativePosition: {
            ...
            progressionWeight: clamp(index / Math.max(total - 1, 1)),
        },
        semanticIdeas: parsed.semanticIdeas ?? [],
        semanticRelationships: parsed.semanticRelationships ?? [],
        semanticSummary: {
            dominantTheme: parsed.semanticSummary?.dominantTheme ??
                'Unknown',
            complexityScore: clamp(parsed.semanticSummary?.complexityScore ?? 0.5),
            emotionalIntensity: clamp(parsed.semanticSummary?.emotionalIntensity ?? 0.5),
            visualDensity: clamp(parsed.semanticSummary?.visualDensity ?? 0.5),
            recommendedMaxVisualIdeas: parsed.semanticSummary
...
// --------------------------------------------------------
//
function createFallbackExtraction(segment, index, total) {
    return {
        sentenceId: segment.id,
        sentenceText: segment.text,
        timing: {
        ...
        narrativePosition: {
            section: determineNarrativeSection(index, total),
            progressionWeight: index / Math.max(total - 1, 1),
        },
        semanticIdeas: [
            {
                id: 'fallback_idea',
...
async function semanticExtractionPipeline(segments) {
    ...
    for (let i = 0; i < segments.length; i++) {
        ...
        try {
            console.log(`Semantic extraction: ${i + 1}/${segments.length}`);
            const extraction = await extractSemanticMeaning(segment, i, segments.length);
            results.push(extraction);
        }
        catch (err) {
            console.error('Semantic extraction failed:', segment.text, err);
            results.push(createFallbackExtraction(segment, i, segments.length));
        }
    }
    //
    // SAVE OUTPUT
