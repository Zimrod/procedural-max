...
// ============================================================
//
async function generateShotPlan(beat, decision) {
    const prompt = `
...
`;
    const response = await openai.chat.completions.create({
        ...
        messages: [
            {
                role: 'system',
                content: 'You are an expert cinematic storyboard AI.',
            },
            {
                role: 'user',
                content: prompt,
...
export async function cinematicPlanner(input) {
    ...
    const validDecisions = Array.isArray(decisions)
        ...
        : [];
    const decisionMap = new Map(validDecisions.map((d) => [d.beatId, d]));
    const plans = [];
    //
    // ==========================================================
    // PROCESS BEATS
    ...
    for (const beat of beats) {
        ...
        // --------------------------------------------------------
        //
        const aiPlan = await generateShotPlan(beat, decision);
        const shots = aiPlan.shots ?? [];
        if (!shots.length) {
            continue;
        }
        //
        ...
        // --------------------------------------------------------
        //
        const ratioTotal = shots.reduce((sum, shot) => sum + (shot.durationRatio ?? 0), 0);
        const normalizedShots = shots.map((shot) => ({
            ...shot,
            normalizedRatio: (shot.durationRatio ?? 0) /
                ratioTotal,
        }));
        //
        // --------------------------------------------------------
        ...
        // --------------------------------------------------------
        //
        const beatStart = beat.timing.startSec;
        const beatEnd = beat.timing.endSec;
        const totalDuration = beatEnd - beatStart;
        let cursor = beatStart;
        const cinematicShots = [];
        normalizedShots.forEach((shot, index) => {
            const isLast = index ===
            ...
            cursor = endSec;
            cinematicShots.push({
                shotId: `${beat.beatId}_shot_${index + 1}`,
                sourceBeatId: beat.beatId,
                timing: {
                    startSec,
                    endSec,
                    startFrame: secondsToFrames(startSec),
                    endFrame: secondsToFrames(endSec),
                    durationFrames: secondsToFrames(durationSec),
                },
                shotPurpose: shot.shotPurpose,
                composition: shot.composition,
                cameraPlan: {
                    behavior: shot.cameraPlan
                        ?.behavior ??
                        'static',
                    intensity: clamp(shot.cameraPlan
                        ?.intensity ?? 0.5, 0, 1),
                    shake: shot.cameraPlan
                        ?.shake ?? 'none',
                },
                lightingStyle: shot.lightingStyle,
                transitionIn: shot.transitionIn,
                transitionOut: shot.transitionOut,
                environmentBehavior: shot.environmentBehavior,
                focalElements: shot.focalElements ?? [],
                emotionalGoal: shot.emotionalGoal ?? '',
        ...
        plans.push({
            beatId: beat.beatId,
            cinematicLanguage: aiPlan.cinematicLanguage ??
                'hybrid',
            energyCurve: clamp(aiPlan.energyCurve ?? 0.5, 0, 1),
...
//# sourceMappingURL=cinematicPlanner.js.map