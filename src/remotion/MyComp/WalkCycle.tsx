...
const Part: React.FC<{
  data: PartData;
  scale: number;
  anchorX: number;      // world position of the part's attachment pivot
  anchorY: number;
  rotateDeg: number;
  pivotId?: string;     // which pivot to use as the rotation center (defaults to first pivot)
}> = ({ data, scale, anchorX, anchorY, rotateDeg, pivotId }) => {
  // Determine which pivot to use for rotation
...
export const WalkCycle: React.FC<{
...
}> = ({ cycleDurationSeconds = 1, characterScale = 1 }) => {
  ...
  parts.lower_leg_r!.viewBox.h; // 99.89 + 179.37 + 113.46 + 113.46 = 506.18

  const baseScale = CANVAS_SIZE / TOTAL_BODY_HEIGHT;
  const finalScale = baseScale * characterScale;

  // Helper to get a pivot from a part
...
// Fallback shoulder positions (right, left) relative to pelvis
...
// Upper arm pivots – these must exist in the part
...
const lowerLegLKneePivot = getPivot(parts.lower_leg_l!, 'pivot_knee_l');
  ...
  // ----- World anchor points (root: pelvis) -----
  const pelvisWorld = {
    x: CANVAS_SIZE * 0.5,
    y: CANVAS_SIZE * 0.6,
  };

  // Torso is placed so that its pelvis pivot aligns with pelvisWorld
  const torsoWorld = pelvisWorld; // the torso's pivot_pelvis will be placed here

  // ----- Animation angles (walk cycle) -----
  const legSwing = 40;
  const legBend = 45;
  const armSwing = 30;
  const forearmHang = 15;
  const torsoSway = 4;
  const headBob = 3;
  ...
  const lowerArmR_rot = forearmHang + clamp(osc(t, 0.5) * 30, -10, 30);
  const lowerArmL_rot = forearmHang + clamp(osc(t, 0) * 50, -10, 30);

  const torso_rot = osc(t, 0.25) * torsoSway;
  const head_rot = osc(t * 2, 0) * headBob;

  // Shoulder world positions (offsets from torso's shoulder pivots)
  // Since the torso is rotated, we need to compute the rotated positions of its shoulder pivots.
  // We'll calculate the world position of shoulder pivot by rotating the offset from pelvis pivot.
  // Use the actual torso rotation to keep shoulders connected as the torso rotates
  ...
  // Head is placed at neck world position, using its own neck pivot
  const headWorld = neckWorld;

  // ----- Compute world positions of elbow/knee using rotation of the limb around its attachment pivot -----
  // Upper arm: rotate the vector from shoulder pivot to elbow pivot
  ...
  // Lower arm: rotates around elbow pivot
  ...
  const rotatedLowerArmR = rotatePoint({ x: 0, y: 0 }, lowerArmRVector, lowerArmR_rot);
  const handRWorld = {
  ...
  const rotatedLowerArmL = rotatePoint({ x: 0, y: 0 }, lowerArmLVector, lowerArmL_rot);
  const handLWorld = {
  ...
  return (
    ...
    >
      {/* LAYER 1 — back/right limbs (far side, rendered first = behind) */}
      <Part
      ...
      {/* LAYER 3 — front/left limbs (near side, rendered after torso = in front) */}
      <Part
      ...
      {/* LAYER 4 — head (always on top of everything) */}
      <Part
