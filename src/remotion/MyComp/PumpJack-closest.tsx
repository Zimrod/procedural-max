...
export const PumpJack: React.FC<{
  ...
  // ── Step 1: Samson post (static) ──────────────────────────────────────
  const samsonPostPivot = getPivot(parts.samson_post!, 'pivot_samson_post');
  ...
  // ── Step 2: Motor – placed using shared pivot_samson_motor ────────────
  // First, get the motor's pivot_samson_motor (where it attaches to the samson post)
  const motorSamsonPivot = getPivot(parts.motor!, 'pivot_samson_motor');
  // Get the motor's crank pivot (pivot_motor)
  const motorCrankPivot = getPivot(parts.motor!, 'pivot_motor');

  // Try to get the samson post's pivot_samson_motor (shared with motor)
  ...
  if (samsonMotorPivotLocal) {
    // Fully pivot‑driven: place motor's samson pivot at samson_post's samson_motor pivot
    ...
    // Then the crank center is offset from that by the motor's local vector
    crankCenterWorld = {
    ...
    // Fallback: motor sits to the right of samson post, at ground level
    // Use motor's own pivot_motor as the crank center
    crankCenterWorld = {
  ...
  // ── Step 3: Counter weight – attached to crank center ───────────────────
  const cwMotorPivot = getPivot(parts.counter_weight!, 'pivot_motor');
  const cwCrankPivot = getPivot(parts.counter_weight!, 'pivot_pitman_arm');
  ...
  // ── Step 4: Walking beam (remains same) ────────────────────────────────
  const beamSamsonPivot = getPivot(parts.walking_beam!, 'pivot_samson_post');
  const beamWalkingPivot = getPivot(parts.walking_beam!, 'pivot_walking_beam');
  const beamArmOffset = {
    x: (beamWalkingPivot.x - beamSamsonPivot.x) * finalScale,
    y: (beamWalkingPivot.y - beamSamsonPivot.y) * finalScale,
  };
  const naturalBeamAngle = Math.atan2(beamArmOffset.y, beamArmOffset.x);
  const beamToCrankDx = crankPinWorld.x - samsonPostWorld.x;
  const beamToCrankDy = crankPinWorld.y - samsonPostWorld.y;
  const targetAngle = Math.atan2(beamToCrankDy, beamToCrankDx);
  const beamRotDeg = ((targetAngle - naturalBeamAngle) * 180) / Math.PI;
  const rotatedBeamArm = rotatePoint({ x: 0, y: 0 }, beamArmOffset, beamRotDeg);
  const beamWalkingPivotWorld = {
    x: samsonPostWorld.x + rotatedBeamArm.x,
    y: samsonPostWorld.y + rotatedBeamArm.y,
  };

  // ── Step 5: Pitman arm (remains same) ──────────────────────────────────
  const pitmanPivotBottom = getPivot(parts.pitman_arm!, 'pivot_pitman_arm');
  const pitmanPivotTop = getPivot(parts.pitman_arm!, 'pivot_walking_beam');
  const pitmanNaturalAngle = Math.atan2(
    (pitmanPivotTop.y - pitmanPivotBottom.y) * finalScale,
    (pitmanPivotTop.x - pitmanPivotBottom.x) * finalScale
  );
  const pitmanTargetAngle = Math.atan2(
    beamWalkingPivotWorld.y - crankPinWorld.y,
    beamWalkingPivotWorld.x - crankPinWorld.x
  );
  const pitmanRotDeg = ((pitmanTargetAngle - pitmanNaturalAngle) * 180) / Math.PI;
