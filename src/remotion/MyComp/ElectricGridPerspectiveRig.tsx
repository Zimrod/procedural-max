// src/remotion/MyComp/ElectricGridPerspectiveRig.tsx
...
const ElectricityPylon: React.FC<PylonProps> = ({
  ...
  return (
    <g opacity={towerSpring} style={{ transformOrigin: `${xAnchor}px ${yBaseline}px`, transform: `scaleY(${towerSpring})` }}>
      ...
      <line x1={xAnchor - w/2} y1={yBaseline} x2={xAnchor - w/6} y2={topY} stroke="#1e2530" strokeWidth={4 * scale} />
      <line x1={xAnchor + w/2} y1={yBaseline} x2={xAnchor + w/6} y2={topY} stroke="#1e2530" strokeWidth={4 * scale} />

      {/* INTERNAL TRUSS LATTICE GIRDER BRACING MAPS */}
      <line x1={xAnchor - w/3} y1={yBaseline - h*0.3} x2={xAnchor + w/3} y2={yBaseline - h*0.3} stroke="#1a202c" strokeWidth={2 * scale} />
      <line x1={xAnchor - w/4} y1={yBaseline - h*0.6} x2={xAnchor + w/4} y2={yBaseline - h*0.6} stroke="#1a202c" strokeWidth={2 * scale} />
      <line x1={xAnchor - w/5} y1={yBaseline - h*0.8} x2={xAnchor + w/5} y2={yBaseline - h*0.8} stroke="#1a202c" strokeWidth={2 * scale} />
      
      {/* Diagonal X-Bracing Lattice */}
      <line x1={xAnchor - w/2} y1={yBaseline} x2={xAnchor + w/3} y2={yBaseline - h*0.3} stroke="#11151d" strokeWidth={1.5 * scale} />
      <line x1={xAnchor + w/2} y1={yBaseline} x2={xAnchor - w/3} y2={yBaseline - h*0.3} stroke="#11151d" strokeWidth={1.5 * scale} />
      <line x1={xAnchor - w/3} y1={yBaseline - h*0.3} x2={xAnchor + w/4} y2={yBaseline - h*0.6} stroke="#11151d" strokeWidth={1.5 * scale} />
      <line x1={xAnchor + w/3} y1={yBaseline - h*0.3} x2={xAnchor - w/4} y2={yBaseline - h*0.6} stroke="#11151d" strokeWidth={1.5 * scale} />

      {/* THREE HIGH-VOLTAGE TRANSMISSION CROSSARMS (Wire Anchors - Resided Left) */}
      {/* Upper Crossarm */}
      <line x1={xAnchor - w*0.8} y1={topY + h*0.15} x2={xAnchor + w*0.8} y2={topY + h*0.15} stroke="#1e2530" strokeWidth={3 * scale} />
      {/* Middle Crossarm */}
      <line x1={xAnchor - w} y1={topY + h*0.35} x2={xAnchor + w} y2={topY + h*0.35} stroke="#1e2530" strokeWidth={3 * scale} />
      {/* Lower Crossarm */}
      <line x1={xAnchor - w*0.7} y1={topY + h*0.55} x2={xAnchor + w*0.7} y2={topY + h*0.55} stroke="#1e2530" strokeWidth={3 * scale} />

      {/* CORE TOP TERMINAL SPOOL HUB */}
      <circle cx={xAnchor} cy={topY} r={6 * scale} fill="#2d3748" />
    </g>
  );
};

export const ElectricGridPerspectiveRig: React.FC<{ accentColor?: string }> = ({
  accentColor = "#48bb78", // Signature Light Green surge hue
}) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  ...
  const pylons = [
    ...
    { x: 540,  y: groundY - 135,  s: 0.41, delay: 30 },  // P4: Center Left (New)
    { x: 350,  y: groundY - 160,  s: 0.30, delay: 40 },  // P5: Midground Left (New)
    { x: 200,  y: groundY - 180,  s: 0.22, delay: 50 },  // P6: Far Background Left (New)
  ];

  // Dynamic calculations generating explicit transmission arm connection nodes
  // Arm index offsets: 0 = Top, 1 = Mid, 2 = Bottom
  const getWireNode = (p: typeof pylons[0], armIndex: number) => {
    const h = 480 * p.s;
    const w = 140 * p.s;
    const topY = p.y - h;
    
    // Multipliers for left-side arm extensions across coordinates
    const offsets = [
      { dx: -w * 0.8, dy: h * 0.15 },
      { dx: -w * 1.0, dy: h * 0.35 },
      { dx: -w * 0.7, dy: h * 0.55 }
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: '#05070a', overflow: 'hidden' }}>
      ...
      {/* TRANSMISSION GLOW DEF STATIONS */}
      ...
      <svg viewBox="0 0 1920 1080" style={{ width: '100%', height: '100%' }}>
        ...
        {/* TECH REVENUE GRID OVERLAYS */}
        <g opacity="0.03">
          {Array.from({ length: 15 }).map((_, i) => (
            <line key={`v-grid-${i}`} x1={i * 140} y1="0" x2={i * 140} y2="1080" stroke="#fff" strokeWidth="1" />
          ))}
        </g>
        ...
        {/* RENDER PHASE 1: INDIVIDUAL CHASSIS STRUCTURAL TOWERS (Back to Front for Proper Z-Index) */}
        {[5, 4, 3, 2, 1, 0].map((idx) => (
          <ElectricityPylon 
            key={`pylon-${idx}`}
            scale={pylons[idx].s} 
        ...
        {globalIntro > 0.4 && [0, 1, 2].map((armIdx) => {
          // Track wire node locations down the complete 6-tower infrastructure array
          ...
          // Infinite Horizon Vanishing Target Calculations extending cleanly to viewport edges
          const leftEdgeNode = { x: -150, y: n6.y + (n6.y - n5.y) * 1.1 };
          const rightEdgeNode = { x: 2100, y: n1.y - (n2.y - n1.y) * 0.85 };

          // Segment components bridging out along the complete canvas view path
          const spans = [
          ...
          return (
            <g key={`grid-circuit-${armIdx}`} opacity={globalIntro}>
              ...
              {spans.map((spanPath, sIdx) => {
                // Approximate a localized perspective scale factor based on progress down the segment line
              ...
              <g filter="url(#coronaDischarge)">
                {spans.map((spanPath, sIdx) => {
                  // Offset each separate section's wave phase to present a continuous flowing current line
        ...
        <text x="1850" y="1020" fill="#4a5568" fontSize="14" fontWeight="700" textAnchor="end" letterSpacing="4">
