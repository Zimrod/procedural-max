    ...
    <div className="w-full lg:w-[420px] lg:float-left bg-[#1e1e1e] rounded-2xl border border-neutral-800 p-5 shadow-2xl shadow-black/60">
      ...
      <div className="flex items-center justify-between mb-4">
        {onOpenDashboard && (
          <button
            ...
            className="flex items-center gap-2 px-3 py-1.5 bg-[#141414] hover:bg-neutral-800 border border-neutral-800 rounded-lg text-xs font-semibold text-neutral-300 hover:text-white transition-all shadow-sm"
          >
      ...
      <div className="flex border border-neutral-800 mb-5 p-1 bg-[#141414] rounded-xl text-center">
        {(["generate", "custom-script", "upload-voiceover"] as const).map((tab) => (
          <button
            ...
            className={`flex-1 py-2 text-[11px] font-semibold tracking-wide rounded-lg transition-all ${
      ...
      {leftTab === "generate" && (
        <div className="space-y-4">
          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-neutral-400 mb-2">Conceptual Prompt Idea</label>
            <textarea
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              placeholder="Describe the type of script you want created..."
              className="w-full min-h-[90px] p-3.5 bg-[#141414] border border-neutral-800 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/50 text-neutral-100 placeholder:text-neutral-600 resize-none transition-all"
            />
          </div>
          <button
          ...
          {aiScript && (
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-emerald-400 mb-2">Editable Generated Script</label>
              <textarea
                value={aiScript}
                onChange={(e) => setAiScript(e.target.value)}
                className="w-full min-h-[140px] p-3.5 bg-[#141414] border border-emerald-900/60 rounded-xl text-sm font-mono focus:outline-none text-neutral-100 transition-all"
              />
            </div>
          )}
          {aiAudioUrl && (
            <div className="rounded-xl border border-neutral-800 bg-[#141414] p-4 mt-2">
              ...
              <audio controls src={`${aiAudioUrl}?v=${aiAudioVersion}`} className="mt-2 w-full h-8 accent-emerald-500" />
            </div>
          )}
        </div>
      )}
      ...
      {leftTab === "custom-script" && (
        <div className="space-y-4">
          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-neutral-400 mb-2">Custom Script Track</label>
            <textarea
              value={customScript}
              onChange={(e) => setCustomScript(e.target.value)}
              placeholder="Paste your script directly here..."
              className="w-full min-h-[185px] p-3.5 bg-[#141414] border border-neutral-800 rounded-xl text-sm font-mono text-neutral-100 focus:outline-none transition-all"
            />
          </div>
          {customAudioUrl && (
            <div className="rounded-xl border border-neutral-800 bg-[#141414] p-4 mt-2">
              <h3 className="text-[10px] font-bold uppercase text-neutral-400">Expert Voiceover Preview</h3>
              <audio controls src={`${customAudioUrl}?v=${customAudioVersion}`} className="mt-2 w-full h-8 accent-emerald-500" />
            </div>
          )}
        </div>
      )}
      ...
      {leftTab === "upload-voiceover" && (
        <div className="space-y-4">
          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-neutral-400 mb-2">Upload Audio File (.mp3, .wav, .m4a)</label>
          ...
          {uploadedAudioUrl && (
            <div className="rounded-xl border border-neutral-800 bg-[#141414] p-4">
              ...
              <audio controls src={`${uploadedAudioUrl}?v=${uploadedAudioVersion}`} className="mt-2 w-full h-8 accent-emerald-500" />
            </div>
          )}
          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-emerald-400 mb-2">Editable Auto-Transcript</label>
            <textarea
              value={uploadedScript}
              onChange={(e) => setUploadedScript(e.target.value)}
              className="w-full min-h-[140px] p-3.5 bg-[#141414] border border-emerald-900/60 rounded-xl text-sm font-mono text-neutral-100 transition-all"
            />
          </div>
        </div>
      )}
