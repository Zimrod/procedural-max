...
export type CountryDropPinProps = {
  country?: string;
  ...
  // Dimensional overrides (defaults automatically to Remotion composition width/height)
  width?: number;
  height?: number;

  // General Canvas & Map Styling Props
  ...
  // Label Position & Typography Styling Props
...
const requirePivot = (asset: ParsedSvgAsset, id: string) => {
  ...
  // Keep the scene renderable when a map export omitted a pivot. The fallback
  // is deterministic and is replaced automatically when the exact pivot exists.
...
export const CountryDropPinRig: React.FC<CountryDropPinProps> = ({
  country = "zimbabwe",
  zoomStartFrame = 20,
  ...
  pinShadow = "drop-shadow(0 12px 18px rgba(0, 0, 0, 0.45)) drop-shadow(0 4px 6px rgba(0, 0, 0, 0.25))",
  labelPosition = "right",
  labelOffset = 28,
  labelColor = "#ffffff",
  labelFontFamily = "Aptos, Inter, sans-serif",
  labelFontSize,
  labelFontWeight = 800,
  labelLetterSpacing = "-0.02em",
  labelTextShadow = "0 4px 12px rgba(0,0,0,0.6)",
  labelTextTransform = "none",
}) => {
  const frame = useCurrentFrame();
  const {fps, width: configWidth, height: configHeight} = useVideoConfig();

  // Responsive Canvas Dimensions derive dynamically from composition context if not overridden
  const width = customWidth ?? configWidth;
  const height = customHeight ?? configHeight;

  // Responsive font size calculation scaled to composition viewport height if unspecified
  const responsiveFontSize = labelFontSize ?? Math.max(16, Math.round(height * 0.044));
  ...
  const geometry = useMemo(() => {
    ...
    // Aspect ratio responsive fit calculation using active width and height bounds
    const fitWorldScale =
    ...
    <AbsoluteFill style={{backgroundColor, overflow: "hidden"}}>
      ...
      <div
        style={{
          ...
          opacity: interpolate(zoomProgress, [0.7, 1], [0, focusedCountryOpacity], {
            extrapolateLeft: "clamp",
            extrapolateRight: "clamp",
          }),
