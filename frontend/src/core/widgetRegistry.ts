...
export const widgetRegistry: Partial<Record<WidgetType, WidgetRegistryEntry>> = {
  ...
  PALLET: {
    ...
    defaultProps: { x: 400, y: 400, scale: 1, rotateDeg: 0, opacity: 1, parentId: '', parentMode: 'all', parentKeyframes: [], transformKeyframes: [] },
    editorFields: [
      field('x', 'X Position', 'number', undefined, 400),
      field('y', 'Y Position', 'number', undefined, 400),
      field('scale', 'Scale', 'number', undefined, 1),
      field('rotateDeg', 'Rotation (°)', 'number', undefined, 0),
      field('opacity', 'Opacity', 'number', undefined, 1),
      field('parentId', 'Parent Asset', 'select'),
      field('parentMode', 'Parent Transform', 'select', ['all', 'position', 'scale', 'rotation', 'opacity'], 'all'),
      field('parentKeyframes', 'Parent Keyframes', 'array'),
      field('transformKeyframes', 'Transform Keyframes', 'array'),
    ],
    buildFallbackProps: ({ extractedData }) => ({
      x: extractedData?.x ?? extractedData?.position?.x ?? 400,
      y: extractedData?.y ?? extractedData?.position?.y ?? 400,
